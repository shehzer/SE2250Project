...
public class Enemy2 : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if (player.isAttacking && !deadState && m_timeSinceAttack > 0.4f && Mathf.Abs(this.transform.position.x - playerObject.transform.position.x) < xOffsetHitbox)
        {
        ...
        if (player.isAttacking && !deadState && m_timeSinceAttack > 0.4f && Mathf.Abs(this.transform.position.x - playerObject.transform.position.x) < xOffsetHitbox)
        {
            ObjectTakeDamage(25, false);
            m_body2d.velocity = new Vector2(m_facingDirection * 3f, 2f);
        } else if (player.isBlocking && player.shieldCombatMode && !deadState && m_timeSinceAttack > 0.4f && Mathf.Abs(this.transform.position.x - playerObject.transform.position.x) < xOffsetHitboxShieldAttack) {
            ObjectTakeDamage(15, true);
            DealDamageToPlayer(10);
            m_body2d.velocity = new Vector2(m_facingDirection * 2f, 1.5f);
        }
